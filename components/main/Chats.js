...
const Chats = () => {
    ...
    const handleSubtitle = (chat) => {
        if (chat.data().messages.length === 0) {
            ...
            const user = chat.data().messages[0].user;
            const firstName = user && user.name ? user.name.substring(0, user.name.indexOf(' ')) : '';

            if (chat.data().groupName != '') { //It is group chat
                if (auth?.currentUser?.email == user._id) { //Yours message
                    //Check length of the last message and add three dots...
            ...
            } else { //It is not a group chat
                if (auth?.currentUser?.email == user._id) { //Your message
                    //Check length of the last message and add three dots...
                    if (chat.data().messages[0].image && chat.data().messages[0].image != '') {
                        return 'You sent an image';
                    } else if (chat.data().messages[0].text.length > 20) {
